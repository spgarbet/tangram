...
```{r pbc, comment=""}
summary(tangram("drug ~ bili + albumin + stage::Categorical + protime + sex + age + spiders", pbc, id="joe", caption="Context Aware Compilation"))
```
...
tangram("drug ~ bili[2] + albumin + stage::Categorical + protime + sex + age + spiders",
        ...
        quant=seq(0, 1, 0.25),
        style="hmisc",
        caption = "HTML5 Table Hmisc Style")
```
...
tangram("drug ~ bili[2] + albumin + stage::Categorical + protime + sex + age + spiders", pbc,
        style="nejm", caption = "HTML5 Table NEJM Style", id="tbl3")
```
...
```{r, results="asis"}
tangram("drug ~ bili[2] + albumin + stage::Categorical[1] + protime + sex[1] + age + spiders[1]", 
        id="tbl4",
        data=pbc,
        pformat = 5,
        style="lancet",
        caption = "HTML5 Table Lancet Style"
       )
```
...
tangram(1 ~ x+y,
        ...
        inline="lancet", caption="Intercept Example", id="tbl5")
```
...
tangram(event ~ id["%1.0f"], df, id="tbl7", summarize_count)
```
